...
public class FeatureToggleService : IFeatureToggleService
{
    ...
    public FeatureToggleService()
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.Log(ELogLevel.Warn, $"Failed to instantiate FeatureToggleService: {ex.Message}");
            _context = null;
        }
    }
    ...
    public Task<bool> IsFeatureEnabled(string flagName)
    {
        ...
        Logger.Log(ELogLevel.Debug, "Starting - [method:IsFeatureEnabled] [class:FeatureToggleService]");
        ...
        try
        {
            ...
            Logger.Log(ELogLevel.Info, $"Success - [method:IsFeatureEnabled] with reading flag-{flagName}.");
            return Task.FromResult(feature.IsEnabled);
        }
        catch (Exception ex)
        {
            // Log warning if evaluation of feature flag fails
            Logger.Log(ELogLevel.Warn, $"Failed - [method:IsFeatureEnabled] on flag-{flagName} with error-message: {ex.Message}");
            return Task.FromResult(false);
        }
    }
}