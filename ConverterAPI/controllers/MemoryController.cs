...
public class MemoryController : ControllerBase
{
    ...
    public async Task<ActionResult<List<string>>> GetMemory()
    {
        ...
        Logger.Log(ELogLevel.Debug, "Starting - [method:GetMemory] [class:MemoryController]");
        ...
        try
        {
            ...
            Logger.Log(ELogLevel.Info, "Success - [method:GetMemory] with fetching latest logs from memory.");
            return Ok(logs);
        }
        catch (Exception ex)
        {
            // Log error if any issue arises during fetching logs
            Logger.Log(ELogLevel.Error, $"Failed - [method:GetMemory] with error-message: {ex.Message}");
